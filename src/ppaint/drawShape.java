...
public class drawShape
{
    ...
    public  drawShape(Point startDrag, Point endDrag, int shapeID)
    {
        switch (shapeID)
        {
            ...
            case 3: //Circle
                r = new Ellipse2D.Double(Math.min(startDrag.x, endDrag.x), Math.min(startDrag.y, endDrag.y), Math.max(Math.abs(startDrag.x - endDrag.x), Math.abs(startDrag.y - endDrag.y)),Math.max(Math.abs(startDrag.x - endDrag.x), Math.abs(startDrag.y - endDrag.y)));
                break;
